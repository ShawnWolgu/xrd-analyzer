...
|---|---|---|---|
| Partial numeric input row | Each two-column row is converted completely before either value is appended | Malformed rows no longer leave `x` and `y` with different lengths | Resolved with an exact atomic-row parsing test |
